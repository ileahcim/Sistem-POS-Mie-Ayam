...
// docblock and escpos.ts. The warung identity on the sample comes from the
// Setting table (passed in by the server page), never hardcoded.
export function PrintPreviewScreen({ settings }: { settings: StoreSettings }) {
  ...
  async function printToDevice(kind: "receipt" | "packing") {
    ...
    try {
      const result =
        kind === "receipt" ? await printTest(driver, settings) : await printPackingListTest(driver, settings);
      if (!result.ok) {
  ...
  return (
    <div className="bg-canvas mx-auto flex min-h-dvh max-w-md flex-col gap-4 p-6">
      ...
      <Button variant="primary" size="large" onClick={() => mockPrinter.printReceipt(buildTestReceipt(settings))}>
        Preview Struk Pembayaran
      </Button>
      <Button variant="ghost" size="large" onClick={() => mockPrinter.printPackingList(buildTestPackingList(settings))}>
