...
- **Sheet & popup** (permintaan pemilik — semua `Sheet`: Menu, add-on Kasir, popup konfirmasi Batalkan/Void, Pisahkan & Bayar, sheet Note): kontainer muncul dengan spring ~0.4s dari `y:-5, scale:0.95, blur(10px), opacity:0`; item di dalamnya muncul berurutan (`delay: index × 0.05`, dari `x:10, scale:0.95, blur`); item yang bisa di-tap punya hover (tint background, 0.4s) dan tap (scale 0.95, 0.2s). Semua di `src/components/ui/sheet-motion.tsx` (`sheetPanelMotion`, `SheetItem`, `HoverTint`) — pakai itu, jangan bikin variasi sendiri. Add-on sheet membatasi urutan stagger di indeks 8 supaya produk dengan banyak opsi tidak bikin kasir menunggu.
